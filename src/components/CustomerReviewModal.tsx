...
import { X, Star, Upload, Camera, Play, Check, User, Mail, Phone, Image as ImageIcon, Video, MessageCircle } from 'lucide-react';
...
export const CustomerReviewModal: React.FC<CustomerReviewModalProps> = ({
    ...
    try {
      ...
      for (const file of imageFiles.slice(0, remaining)) {
        if (file.size > 10 * MAX_IMAGE_SIZE) {
          showError(`${file.name} es muy grande`);
          continue;
        }
  ...
  }, [formData.images.length, showError, success]);
  ...
  const handleVideoUpload = useCallback(async (files: FileList | null) => {
    ...
    if (remaining <= 0) {
      showError(`Máximo ${MAX_VIDEOS} videos permitidos`);
      return;
    }
    ...
    try {
      ...
      for (const file of videoFiles.slice(0, remaining)) {
        if (file.size > MAX_VIDEO_SIZE) {
          showError(`${file.name} excede 2MB`);
          continue;
        }
      ...
      if (newVideos.length > 0) {
        ...
        success(`${newVideos.length} video(s) subido(s)`);
      }
    } catch {
      showError('Error al subir videos');
    }
  }, [formData.videos.length, showError, success]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.comment || formData.comment.length < 10) {
      showError('Escribe una reseña de al menos 10 caracteres');
      return;
    }
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <ImageIcon className="w-6 h-6 mx-auto mb-2 text-zinc-400" />
