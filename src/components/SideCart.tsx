...
export const SideCart: React.FC<SideCartProps> = ({ isOpen, onClose }) => {
  const { cart: items, removeItem: removeFromCart, updateQuantity, freeShippingProgress, remainingForFreeShipping, appliedCoupon, setAppliedCoupon, discount, finalTotal } = useCart();
  ...
  const applyCoupon = () => {
    ...
    if (total < coupon.minPurchase) {
      setCouponError(`Compra mínima $${formatCOP(coupon.minPurchase)} para este cupón`);
      return;
    }
    ...
    success(`Cupón ${coupon.code} aplicado`);
    setCouponInput('');
  };
