...
export const AdminCouponsScreen = () => {
  ...
  const generateDiscountPreview = (value: number, type: 'percent' | 'fixed', maxDisc?: number) => {
    const testAmount = 100000;
    let discountValue = type === 'percent' ? (testAmount * value) / 100 : value;
    if (maxDisc && type === 'percent') discountValue = Math.min(discountValue, maxDisc);
  ...
  const openModal = (coupon?: Coupon) => {
    if (coupon) {
      ...
      setFormData({
        code: coupon.code,
        type: coupon.type === 'percent' || (coupon.type as string) === 'percentage' ? 'percent' : 'fixed',
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-1 text-2xl font-black text-zinc-900 dark:text-white">
                          ...
                          {coupon.type === 'percent' ? coupon.discount : (coupon.discount || 0).toLocaleString('es-CO')}
                          {coupon.type === 'percent' && <span className="text-base">%</span>}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        generateDiscountPreview(formData.discount, type, formData.maxDiscount || undefined);
                      }}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1.5">
                      Valor {formData.type === 'percent' ? '(%)' : '($ COP)'}
                    </label>
                    <input
                      type="number"
                  ...
                  <div className="p-4 rounded-xl bg-primary/5 dark:bg-primary/10 border border-primary/20">
                    ...
                    <div className="h-1.5 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min((formData.discount / 100) * 100, 100)}%` }}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          generateDiscountPreview(formData.discount, formData.type, maxDisc || undefined);
                        }}
                        className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/30 text-zinc-900 dark:text-zinc-100"
                        min="0"
