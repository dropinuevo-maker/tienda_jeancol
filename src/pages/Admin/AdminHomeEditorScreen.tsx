...
export const AdminHomeEditorScreen = () => {
  ...
  const handleHeroImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setHeroSection((prev: any) => ({ ...prev, image: base64 }));
  ...
  const handleUpdateTrustItem = (index: number, newItem: any) => {
    setTrustItems((prev: any) => {
      const next = [...prev];
      next[index] = newItem;
      return next;
    });
  ...
  const TrustItemCard = ({ item, index, onUpdate, isDark }: { item: any; index: number; onUpdate: any; isDark: boolean }) => (
    <div className={`p-5 rounded-2xl transition-all ${
        ...
        <div className="flex gap-1">
          {ICON_OPTIONS.map(opt => (
            <button
              key={opt.value}
              onClick={() => {
                onUpdate(index, { ...item, icon: opt.value });
              }}
      ...
      <div className="space-y-3">
        ...
        <Input 
          ...
          onChange={(v: string) => {
            onUpdate(index, { ...item, description: v });
          }} 
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input 
                ...
                onChange={(v: string) => setHeroSection((p: any) => ({ ...p, badgeText: v }))}
                placeholder="Ej: Nueva Colección 2024"
              />
              <Input 
                label="Texto en Vivo" 
                value={heroSection.liveText} 
                onChange={(v: string) => setHeroSection((p: any) => ({ ...p, liveText: v }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input 
                ...
                onChange={(v: string) => setHeroSection((p: any) => ({ ...p, title: v }))}
                placeholder="Ej: Define tu Legado"
              />
              <Input 
                label="Texto del Botón" 
                value={heroSection.buttonText} 
                onChange={(v: string) => setHeroSection((p: any) => ({ ...p, buttonText: v }))}
            ...
            <Input 
              ...
              onChange={(v: string) => setHeroSection((p: any) => ({ ...p, subtitle: v }))}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {trustItems.map((item: any, index: number) => (
                <TrustItemCard key={index} item={item} index={index} onUpdate={handleUpdateTrustItem} isDark={isDark} />
              ))}
            </div>
          </div>
        );
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {banners.map((banner: any, index: number) => {
                  ...
                  const selectedProduct = banner.link.startsWith('/product/') 
                    ? products.find((p: any) => p.id === banner.link.split('/').pop()) 
                    : null;
                  return (
                        ...
                        <div className="relative w-full md:w-64 aspect-video md:aspect-square flex-shrink-0 bg-zinc-200 dark:bg-zinc-700">
                          ...
                          <div className="absolute bottom-2 left-2 right-2">
                            <div className="flex gap-1">
                              ...
                              <button
                                onClick={() => {
                                  const newCollections = collections.filter((_: any, i: number) => i !== index);
                                  setCollections(newCollections);
                                }}
                ...
                <div className="bg-white dark:bg-zinc-950 rounded-xl overflow-hidden shadow-2xl">
                  <div className="py-8 px-6 lg:px-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-6 h-auto lg:h-[500px] xl:h-[600px]">
                      ...
                      ) : collections.slice(0, 4).map((collection: any, index: number) => {
                  ...
                  <div className="py-10 px-6 lg:px-12">
                    <div className="text-center space-y-3 mb-10">
                      ...
                      <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-zinc-900 dark:text-white uppercase tracking-tighter italic leading-none">
                        {testimonialsTitle.split(' ').map((word: string, i: number) => (
                          i === 0 ? <span key={i}>{word}</span> : i === 1 ? <span key={i}> <span className="text-primary">{word}</span></span> : <span key={i}> {word}</span>
                        ))}
                      </h2>
                    </div>
