...
export const AdminOrdersScreen = () => {
  ...
  const handleAddProductToOrder = (product: typeof products[0], formType: 'new' | 'edit') => {
    ...
    if (existingIndex >= 0) {
      ...
      setForm((prev: any) => formType === 'new' ? { ...prev, items: updatedItems } : { ...prev!, items: updatedItems });
    } else {
      setForm((prev: any) => formType === 'new' 
  ...
  const handleRemoveItemFromOrder = (index: number, formType: 'new' | 'edit') => {
    const setForm = formType === 'new' ? setNewOrderForm : setEditOrderForm;
    setForm((prev: any) => formType === 'new' 
      ? { ...prev, items: prev.items.filter((_: any, i: number) => i !== index) }
      : { ...prev!, items: prev!.items.filter((_: any, i: number) => i !== index) }
    );
  };
                  ...
                  >
                    ...
                    <div className={`flex items-center justify-between pt-4 ${isDark ? 'border-t border-zinc-800' : 'border-t border-zinc-100'}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`text-[10px] lg:text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
                          {(PAYMENT_LABELS as any)[order.paymentMethod] || order.paymentMethod}
                        </span>
                      </div>
      ...
      <AnimatePresence>
        {editingOrderId && editOrderForm && (
          <OrderFormModal
            title="Editar Pedido"
            form={editOrderForm}
            setForm={setEditOrderForm as any}
      ...
      >
        ...
        <div className={`rounded-xl p-4 mb-6 ${isDark ? 'bg-zinc-800' : 'bg-zinc-50'}`}>
          ...
          <div className={`mt-3 pt-3 border-t ${isDark ? 'border-zinc-700' : 'border-zinc-200'}`}>
            ...
            <span className={`text-xs font-bold ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>{(PAYMENT_LABELS as any)[order.paymentMethod] || order.paymentMethod}</span>
          </div>
        </div>
        ...
        {nextStatuses.length > 0 && (
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              >
                ...
                {nextStatuses.map((status: any) => (
                  <option key={status} value={status}>{(STATUS_CONFIG as any)[status].label}</option>
                ))}
              </select>
            ...
            {selectedNewStatus && (
              <button
                onClick={() => onStatusChange(order.id, selectedNewStatus as OrderStatus)}
              ...
              >
                ...
                <span>Confirmar: {(STATUS_CONFIG as any)[selectedNewStatus]?.label || selectedNewStatus}</span>
              </button>
            )}
          </div>
        )}
        ...
        <div className={`text-[10px] space-y-1 ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
          ...
          {order.updatedAt && <p>Actualizado: {formatDateTime(order.updatedAt)}</p>}
        </div>
...
const OrderFormModal = ({ title, form, setForm, onSubmit, onClose, showProductSelector, setShowProductSelector, onAddProduct, onRemoveItem, onUpdateQuantity, getTotal, isDark }: {
  title: string;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 20 }}
        className={`relative rounded-3xl p-6 lg:p-8 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl ${isDark ? 'bg-zinc-900' : 'bg-white'}`}
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className={`text-xl font-black uppercase tracking-tight ${isDark ? 'text-white' : 'text-zinc-900'}`}>{title}</h2>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Nombre Completo *</label>
              <input
                type="text"
                value={form.customer}
                onChange={(e) => setForm((prev: any) => ({ ...prev, customer: e.target.value }))}
            ...
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Email *</label>
              <input
                type="email"
                value={form.email || ''}
            ...
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Teléfono</label>
              <input
                type="tel"
                value={form.phone || ''}
            ...
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Método de Pago</label>
              <select
                value={form.paymentMethod || 'contraentrega'}
            ...
            <div className="md:col-span-2">
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Dirección</label>
              <input
                type="text"
                value={form.address || ''}
            ...
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Departamento</label>
              <div className="relative">
                <select
                ...
                >
                  ...
                  {COLOMBIAN_DEPARTMENTS.map((dept: any) => (
            ...
            <div>
              <label className={`block text-xs font-black mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Ciudad</label>
              <div className="relative">
                <select
                  value={form.city || ''}
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-xs font-black uppercase tracking-widest ${isDark ? 'text-zinc-400' : 'text-zinc-400'}`}>Productos</h3>
            <button
          ...
          {form.items && form.items.length === 0 ? (
            <div className={`text-center py-8 border-2 border-dashed rounded-xl ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-50 border-zinc-200'}`}>
              <Package className={`w-8 h-8 mx-auto mb-2 ${isDark ? 'text-zinc-600' : 'text-zinc-300'}`} />
              <p className={`text-sm ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>No hay productos añadidos</p>
            </div>
          ) : (
            <div className="space-y-2">
              {form.items && form.items.map((item, idx) => (
                <div key={idx} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-50'}`}>
                  <img src={item.image} alt={item.name} className="w-12 h-12 rounded-lg object-cover" referrerPolicy="no-referrer" />
                  <div className="flex-1 min-w-0">
                    <p className={`font-bold text-sm truncate ${isDark ? 'text-white' : 'text-zinc-900'}`}>{item.name}</p>
                    <p className={`text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>${formatPriceCOP(item.price)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => onUpdateQuantity(idx, item.quantity - 1)}
                      className={`w-8 h-8 rounded-lg font-bold transition-colors ${isDark ? 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600' : 'bg-zinc-200 text-zinc-600 hover:bg-zinc-300'}`}
                    >
                      -
                    </button>
                    <span className={`w-8 text-center font-bold ${isDark ? 'text-white' : 'text-zinc-900'}`}>{item.quantity}</span>
                    <button
                      onClick={() => onUpdateQuantity(idx, item.quantity + 1)}
                      className={`w-8 h-8 rounded-lg font-bold transition-colors ${isDark ? 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600' : 'bg-zinc-200 text-zinc-600 hover:bg-zinc-300'}`}
                    >
                  ...
                  <button onClick={() => onRemoveItem(idx)} className={`p-2 rounded-lg transition-colors ${isDark ? 'text-red-400 hover:bg-red-900/20' : 'text-red-500 hover:bg-red-50'}`}>
        ...
        {/* Notes */}
        <div className="mb-6">
          <label className={`block text-xs font-black uppercase tracking-widest mb-1 ${isDark ? 'text-zinc-400' : 'text-zinc-400'}`}>Notas</label>
          <textarea
            value={form.notes || ''}
        ...
        <div className={`rounded-xl p-4 mb-6 ${isDark ? 'bg-zinc-800' : 'bg-zinc-50'}`}>
          <div className="flex justify-between text-sm mb-2">
            <span className={isDark ? 'text-zinc-500' : 'text-zinc-500'}>Subtotal</span>
            <span className={`font-bold ${isDark ? 'text-white' : 'text-zinc-900'}`}>${formatPriceCOP((form.items || []).reduce((acc, item) => acc + (item.price * item.quantity), 0))}</span>
          </div>
          <div className="flex justify-between text-sm mb-2">
            <span className={isDark ? 'text-zinc-500' : 'text-zinc-500'}>Envío</span>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onSubmit} className="flex-1 py-3 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
...
const ProductSelectorModal = ({ products, uniqueCategories, searchQuery, setSearchQuery, categoryFilter, setCategoryFilter, onSelect, onClose, isDark }: {
  products: any[];
  ...
  <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <motion.div
      ...
      animate={{ opacity: 1, scale: 1, y: 0 }}
      exit={{ opacity: 0, scale: 0.95, y: 20 }}
      className={`relative rounded-3xl p-6 lg:p-8 w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl ${isDark ? 'bg-zinc-900' : 'bg-white'}`}
    >
      <div className="flex items-center justify-between mb-4">
        <h2 className={`text-xl font-black uppercase tracking-tight ${isDark ? 'text-white' : 'text-zinc-900'}`}>Seleccionar Producto</h2>
        <button onClick={onClose} className={`p-2 rounded-full transition-colors ${isDark ? 'hover:bg-zinc-800' : 'hover:bg-zinc-100'}`}>
          <X className={`w-5 h-5 ${isDark ? 'text-zinc-400' : ''}`} />
        </button>
      </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="relative">
          ...
          <input
            ...
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className={`w-full pl-10 pr-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none ${
              isDark ? 'bg-zinc-800 border-zinc-700 text-white placeholder-zinc-500' : 'bg-zinc-50 border-zinc-200 text-zinc-900'
            }`}
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${isDark ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600'}`}>
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      <p className={`text-xs mb-3 ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>{products.length} producto{products.length !== 1 ? 's' : ''} encontrado{products.length !== 1 ? 's' : ''}</p>

      <div className="flex-1 overflow-y-auto">
        {products.length === 0 ? (
          <div className="text-center py-12">
            <Package className={`w-12 h-12 mx-auto mb-3 ${isDark ? 'text-zinc-700' : 'text-zinc-300'}`} />
            <p className={isDark ? 'text-zinc-500' : 'text-zinc-500'}>No se encontraron productos</p>
          </div>
        ) : (
            ...
            {products.map((product) => (
              <button
                ...
                className={`flex items-center gap-3 p-3 rounded-xl transition-colors text-left ${
                  isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-zinc-50 hover:bg-zinc-100'
                }`}
              >
                <img src={product.image} alt={product.name} className="w-14 h-14 rounded-lg object-cover" referrerPolicy="no-referrer" />
                <div className="flex-1 min-w-0">
                  <p className={`font-bold text-sm truncate ${isDark ? 'text-white' : 'text-zinc-900'}`}>{product.name}</p>
                  <p className={`text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>{product.category} | Stock: {product.stock || 0}</p>
                  <p className="text-sm font-black text-primary">${formatPriceCOP(product.offerPrice || product.price)}</p>
                </div>
                <Plus className={`w-5 h-5 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`} />
              </button>
            ))}
          </div>
        )}
