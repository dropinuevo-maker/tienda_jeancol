...
export const AdminReviewsScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <AdminNavigation
        title="Reseñas"
        subtitle={`${pendingReviews.length} pendientes • ${approvedReviews.length} aprobadas`}
        actions={
          <button
            onClick={() => openModal()}
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-4 lg:p-6 space-y-4 lg:space-y-5 overflow-y-auto flex-1">
                ...
                {/* Media Upload */}
                <div>
                  <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1.5">Multimedia (Opcional)</label>
                  <div
                    ...
                    className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${
                      isDragging 
                        ...
                        : 'border-zinc-200 dark:border-zinc-800 hover:border-primary/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
                    }`}
                  >
                    <input
                      type="file"
                    ...
                    <p className="text-sm font-bold text-zinc-900 dark:text-white mb-1">Subir fotos o videos</p>
                    <p className="text-xs text-zinc-500">Arrastra archivos o haz clic para buscar</p>
                  </div>
                  ...
                  {(formData.images.length > 0 || formData.videos.length > 0) && (
                    <div className="grid grid-cols-4 gap-2 mt-4">
                      {formData.images.map((img, i) => (
                        <div key={`img-${i}`} className="relative group aspect-square rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800">
                      ...
                      {formData.videos.map((vid, i) => (
                        <div key={`vid-${i}`} className="relative group aspect-square rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-black">
        ...
        {previewMedia && previewMedia.src && (
          ...
          >
            {previewMedia.type === 'image' ? (
              ...
              <motion.video initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} src={previewMedia.src} controls autoPlay className="max-w-full max-h-full rounded-lg" />
            )}
